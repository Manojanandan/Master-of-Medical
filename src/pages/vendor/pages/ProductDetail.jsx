...
const ProductDetail = () => {
  ...
  useEffect(() => {
    if (currentProduct && categories.length > 0) {
      ...
      // Find category ID from subcategory ID
      const categoryId = findCategoryIdFromSubcategoryId(currentProduct.subCategoryId);
      console.log('Found category ID:', categoryId, 'for subcategory ID:', currentProduct.subCategoryId);
  ...
  // Find category ID from subcategory ID
  const findCategoryIdFromSubcategoryId = (subcategoryId) => {
    if (!subcategoryId || !Array.isArray(categories) || !categories.length) return null;
    
    for (const category of categories) {
      if (Array.isArray(category.SubCategories)) {
        const foundSubcategory = category.SubCategories.find(sub => 
          sub.id === subcategoryId || sub._id === subcategoryId
        );
        if (foundSubcategory) {
          console.log('Found category ID:', category.id, 'for subcategory ID:', subcategoryId);
          return category.id;
        }
      }
    }
    console.log('No category found for subcategory ID:', subcategoryId);
    return null;
  };
